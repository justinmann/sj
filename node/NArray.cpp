...
shared_ptr<CVar> NArray::getVarImpl(Compiler* compiler, CResult& result, shared_ptr<CBaseFunction> thisFunction, shared_ptr<CVar> thisVar, shared_ptr<CVar> dotVar, CTypeReturnMode returnMode) {
    ...
    if (firstElement != nullptr) {
        ...
        auto elementVar = firstElement->getVar(compiler, result, thisFunction, thisVar, CTRM_Heap);
        auto elementType = elementVar->getType(compiler, result, CTRM_Heap);
        ...
        for (auto element : *elements) {
            ...
            statementVars.push_back(setAtItem->getVar(compiler, result, thisFunction, thisVar, nullptr, CTRM_NoPref));
            index++;
        }
        
        // return return array value
        auto arrayVar = make_shared<NVariable>(loc, arrayName.c_str());
        statementVars.push_back(arrayVar->getVar(compiler, result, thisFunction, thisVar, nullptr, CTRM_NoPref));
    }
