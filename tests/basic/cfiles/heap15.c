...
#line 9
    sjf_class_heap(g_c);
#line 6 "heap15.sj"
    g_cb.inner._parent = (sjs_object*)g_c;
#line 6
    g_cb.inner._parent->_refCount++;
#line 6
    g_cb._destroy = (void(*)(sjs_object*))sjf_class_destroy;
#line 6
    g_cb.inner._cb = (void(*)(sjs_object*, int32_t*))sjf_class_func;
    if ((uintptr_t)g_cb.inner._parent > 1) {
        g_cb.inner._parent->_refCount--;
        if (g_cb.inner._parent->_refCount <= 0) {
            g_cb._destroy(g_cb.inner._parent);
            free(g_cb.inner._parent);
        }
    }
