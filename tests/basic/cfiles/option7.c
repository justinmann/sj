...
#line 6 "option7.sj"
        sjs_class* copyoption14 = (g_c._refCount != -1 ? &g_c : 0);
        if (copyoption14 != 0) {
            g_d._refCount = 1;
#line 6 "option7.sj"
            sjf_class_copy(&g_d, copyoption14);
        } else {
            g_d._refCount = -1;
        }
    } else {
...
#line 9
    sjf_console_writeline(sjt_functionParam7);
    main_destroy();
    return 0;
}
