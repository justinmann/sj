...
void sjf_class_destroy(sjs_class* _this) {
    _this->x->_refCount--;
    if (_this->x->_refCount <= 0) {
        weakptr_release(_this->x);
        sjf_string_destroy(_this->x);
        free(_this->x);
    }
}
...
int main(int argc, char** argv) {
    ...
    sjv_c.x = (sjs_string*)malloc(sizeof(sjs_string));
    sjv_c.x->_refCount = 1;
    sjf_string_copy(sjv_c.x, &sjt_call1);
    sjf_class(&sjv_c);
    main_destroy();
    return 0;
}
