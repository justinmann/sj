...
int main(int argc, char** argv) {
    ...
    sjt_call1 = (sjs_func*)malloc(sizeof(sjs_func));
    sjt_call1->_refCount = 1;
    sjf_func_heap(sjt_call1);
    sjv_a._refCount = 1;
    sjf_func_copy(&sjv_a, sjt_call1);
    main_destroy();
    return 0;
}
...
void main_destroy() {
    ...
    if (sjt_call1->_refCount <= 0) {
        ...
        sjf_func_destroy(sjt_call1);
        free(sjt_call1);
    }
    if (sjv_a._refCount == 1) { sjf_func_destroy(&sjv_a); }
;
