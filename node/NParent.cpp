...
shared_ptr<ReturnValue> CParentVar::transpileGet(Compiler* compiler, CResult& result, TrOutput* trOutput, TrBlock* trBlock, shared_ptr<ReturnValue> dotValue, const char* thisName) {
    auto parentTypes = function->getThisTypes(compiler, result);
    if (!parentTypes) {
        return nullptr;
    }
