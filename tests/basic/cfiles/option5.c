...
#line 3
    sjf_func(&sjt_value2);
#line 3
    if (sjv_foo._refCount == 1) { sjf_func_destroy(&sjv_foo); }
;
#line 4
    sjs_func* copyoption14 = &sjt_value2;
    if (copyoption14 != 0) {
        sjv_foo._refCount = 1;
#line 4 "option5.sj"
        sjf_func_copy(&sjv_foo, copyoption14);
    } else {
        sjv_foo._refCount = -1;
    }
    main_destroy();
...
;
    if (sjt_value2._refCount == 1) { sjf_func_destroy(&sjt_value2); }
;
    if (sjv_foo._refCount == 1) { sjf_func_destroy(&sjv_foo); }
;
