...
#define sjs_foo_typeId 22
#define sjs_bar_typeId 21
...
void sjf_func(sjs_bar* b, sjs_foo* _return) {
    sjs_foo sjt_funcold1 = { -1 };

    sjt_funcold1._refCount = 1;
#line 2 "heap3.sj"
    sjf_foo_copy(&sjt_funcold1, &b->f);
#line 2
    sjt_funcold1._refCount = 1;
#line 5
    sjt_funcold1.x = 2;
#line 5
    sjf_foo(&sjt_funcold1);
#line 5
    if (b->f._refCount == 1) { sjf_foo_destroy(&b->f); }
;
#line 2
    sjf_foo_copy(&b->f, &sjt_funcold1);
#line 2
    _return->_refCount = 1;
#line 4
    sjf_foo_copy(_return, &b->f);

    if (sjt_funcold1._refCount == 1) { sjf_foo_destroy(&sjt_funcold1); }
;
}
