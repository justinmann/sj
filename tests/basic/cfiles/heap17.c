...
void sjf_class_func(sjs_class* _parent) {
    ...
    sji_iface sjt_functionParam4 = { 0 };
    sji_iface sjt_void1 = { 0 };

#line 6 "heap17.sj"
    sjt_cast2 = _parent;
#line 1
    sjf_class_as_sji_iface(sjt_cast2, &sjt_functionParam4);
    if (sjt_functionParam4._parent != 0) {
        sjt_functionParam4._parent->_refCount++;
    }

#line 1
    sjf_store_heap(sjt_functionParam4, &sjt_void1);
...
void sjf_store_heap(sji_iface a, sji_iface* _return) {
    if (g_keep._parent != 0) {
        g_keep._parent->_refCount--;
        if (g_keep._parent->_refCount <= 0) {
            g_keep._vtbl->destroy(g_keep._parent);
            free(g_keep._parent);
        }
    }
