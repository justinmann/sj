...
#line 3
    sjt_parent3 = g_a;
#line 13
    sjt_parent3._vtbl->test_heap(sjt_parent3._parent, &g_bob);
    main_destroy();
    return 0;
}
...
void main_destroy() {
    ...
    g_bob->_refCount--;
    if (g_bob->_refCount <= 0) {
        weakptr_release(g_bob);
        sjf_string_destroy(g_bob);
        free(g_bob);
    }
