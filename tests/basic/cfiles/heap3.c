...
#define sjs_foo_typeId 27
#define sjs_bar_typeId 26
...
void sjf_bar_destroy(sjs_bar* _this) {
    if (_this->f._refCount == 1) { sjf_foo_destroy(&_this->f); }
;
}
...
void sjf_func(sjs_bar* b, sjs_foo* _return) {
    sjs_foo sjt_funcold2 = { -1 };

    sjt_funcold2._refCount = 1;
#line 2 "heap3.sj"
    sjf_foo_copy(&sjt_funcold2, &b->f);
#line 2
    sjt_funcold2._refCount = 1;
#line 5
    sjt_funcold2.x = 2;
#line 5
    sjf_foo(&sjt_funcold2);
#line 5
    if (b->f._refCount == 1) { sjf_foo_destroy(&b->f); }
;
#line 2
    sjf_foo_copy(&b->f, &sjt_funcold2);
#line 2
    _return->_refCount = 1;
#line 4
    sjf_foo_copy(_return, &b->f);

    if (sjt_funcold2._refCount == 1) { sjf_foo_destroy(&sjt_funcold2); }
;
}

void sjf_func_heap(sjs_bar* b, sjs_foo** _return) {
    sjs_foo sjt_funcold3 = { -1 };

    sjt_funcold3._refCount = 1;
#line 2 "heap3.sj"
    sjf_foo_copy(&sjt_funcold3, &b->f);
#line 2
    sjt_funcold3._refCount = 1;
#line 5
    sjt_funcold3.x = 2;
#line 5
    sjf_foo(&sjt_funcold3);
#line 5
    if (b->f._refCount == 1) { sjf_foo_destroy(&b->f); }
;
#line 2
    sjf_foo_copy(&b->f, &sjt_funcold3);
#line 2
...
#line 4
    ...
    if (sjt_funcold3._refCount == 1) { sjf_foo_destroy(&sjt_funcold3); }
;
}
...
#line 10
    sjf_func(sjt_functionParam4, &sjt_call1);
    main_destroy();
    return 0;
}
...
void main_destroy() {
    ...
    if (g_b._refCount == 1) { sjf_bar_destroy(&g_b); }
;
