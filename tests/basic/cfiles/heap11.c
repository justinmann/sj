...
void sjf_class2_copy(sjs_class2* _this, sjs_class2* _from) {
#line 7 "heap11.sj"
    sjs_inner* copyoption15 = (_from->inner._refCount != -1 ? &_from->inner : 0);
    if (copyoption15 != 0) {
        _this->inner._refCount = 1;
#line 7 "heap11.sj"
        sjf_inner_copy(&_this->inner, copyoption15);
    } else {
        _this->inner._refCount = -1;
    }
}
...
#line 11 "heap11.sj"
    ...
    if (copyoption14 != 0) {
        g_x3._refCount = 1;
#line 11 "heap11.sj"
        sjf_class_copy(&g_x3, copyoption14);
    } else {
        g_x3._refCount = -1;
    }
...
#line 8
    ...
    if (copyoption16 != 0) {
        sjt_value3.inner._refCount = 1;
#line 8 "heap11.sj"
        sjf_inner_copy(&sjt_value3.inner, copyoption16);
    } else {
        sjt_value3.inner._refCount = -1;
    }

#line 8
    sjf_class2(&sjt_value3);
#line 12
    sjs_class2* copyoption17 = &sjt_value3;
    if (copyoption17 != 0) {
        g_x7._refCount = 1;
#line 12 "heap11.sj"
        sjf_class2_copy(&g_x7, copyoption17);
    } else {
        g_x7._refCount = -1;
    }
    main_destroy();
...
;
    if (g_x3._refCount == 1) { sjf_class_destroy(&g_x3); }
;
    if (g_x7._refCount == 1) { sjf_class2_destroy(&g_x7); }
;
    if (sjt_value1._refCount == 1) { sjf_hash_type_bool_destroy(&sjt_value1); }
;
    if (sjt_value2._refCount == 1) { sjf_class_destroy(&sjt_value2); }
;
    if (sjt_value3._refCount == 1) { sjf_class2_destroy(&sjt_value3); }
;
