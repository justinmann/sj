...
string NCCode::expandMacro(Compiler* compiler, CResult& result, shared_ptr<CBaseFunction> thisFunction, shared_ptr<CVar> thisVar, string macro) {
    ...
    if (functionName.compare("forceParent") == 0) {
        ...
        auto ctype = thisFunction->getVarType(compiler, result, make_shared<CTypeName>(CTC_Value, param));
        if (ctype) {
        ...
        auto ctype = thisFunction->getVarType(compiler, result, make_shared<CTypeName>(CTC_Value, typeName));
        if (ctype) {
